...
FB_SEARCH_ACTOR = "easyapi~facebook-posts-search-scraper"   # searches FB by keyword/hashtag
FB_PAGES_ACTOR  = "apify~facebook-posts-scraper"            # scrapes specific page URLs

# Optional: known Osho Facebook pages to pull posts from directly (most reliable).
# Add page URLs you trust here, e.g. "https://www.facebook.com/OshoInternational/"
FB_PAGES = [
    # "https://www.facebook.com/OshoWorld/",
    # "https://www.facebook.com/OshoInternationalMeditationResort/",
]

# What we look for. Add/remove freely.
IG_HASHTAGS = ["oshomeditation", "osho", "oshocamp", "oshoretreat", "oshoworkshop",
               "dynamicmeditation", "kundalinimeditation", "oshogathering", "sannyas",
               "oshofestival", "oshointernational", "meditationcamp", "oshohimalayas",
               "oshonisarga", "oshmeditationresort"]
...
MAX_POST_AGE_DAYS = 10        # only look at posts from the last N days (lower = cheaper, fresher)
...
def run_actor(actor, payload):
    ...
    try:
        ...
        if items:
            print(f"    sample keys: {list(items[0].keys())[:12]}")
        return items
...
def scrape_instagram():
    ...
    for it in items:
        ...
        link = it.get("url") or it.get("postUrl") or ""
...
def _fb_post(it):
    """Normalise one Facebook dataset item into our shape. Picks the real permalink."""
    cap = it.get("text") or it.get("message") or it.get("message_text") or ""
    if not cap:
        return None
    # prefer the canonical post permalink
    link = it.get("topLevelUrl") or it.get("url") or it.get("postUrl") or it.get("link") or ""
    # image: try several known field shapes across actors
    img = it.get("thumb") or it.get("imageUrl") or it.get("image") or ""
    if not img and isinstance(it.get("media"), list) and it["media"]:
        m0 = it["media"][0]
        img = (m0.get("thumbnail") or m0.get("image") or
               (m0.get("photo_image") or {}).get("uri") or "")
    return {
        "caption": cap,
        "url": link,
        "image": img or "",
...
def scrape_facebook():
    ...
    # A) keyword/hashtag search across Facebook (no fixed pages needed)
    search_payload = {
    ...
    # B) (optional) pull posts directly from trusted Osho pages — most reliable links
    if FB_PAGES:
        page_payload = {
...
def build():
    ...
    print(f"  written (deduped) .... {len(events)}")
    print("="*60)
